(function() {
    ...
    function renderMessages(messages) {
        ...
        chatMessages.innerHTML = messages.map((message) => {
            ...
            const imageHtml = message.image_path
                ? `<img src="/static/uploads/chat/${message.image_path.replace(/^uploads\\/chat\\//, '')}" alt="Chat image" class="chat-image">`
                : '';
        ...
        latestMessageId = messages[messages.length - 1].id || latestMessageId;
        scrollToBottom(true);
    }
    ...
    async function fetchMessages(silent) {
        try {
            ...
            if (newestId !== latestMessageId || !silent) {
